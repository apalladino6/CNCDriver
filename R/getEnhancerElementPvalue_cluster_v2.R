...
#' 
getEnhancerElementPvalueWithPreFilter2<-function(inputFileDir,outputFileDir,
    ...
    for(i in 1:length(groupType)){
      ...
      ####
      # enhancer element section
      ####
      
      geneNameVector<-unique(geneDF$geneSymbol)
      geneDF<-split(geneDF,geneDF$geneSymbol)
      
      geneDFenhancer<-geneDF
      ...
      if(file.exists(fileName)){
        ...
        geneDFcluster<-mclapply(1:length(geneNameVector), function(x){
          ...
          tmp0DF<-geneDFenhancer[[geneName]]
      ...
      requiredCols<-ncol(mergeDF)
      
      mergeDF$posIndex<-paste(mergeDF$chr,":",mergeDF$posStart,"-",mergeDF$posEnd,sep="")
      
      prefilteredDF<-mergeDF[mergeDF$posIndex %in% geneDFunique$posIndex,]
      
      prefilteredDF<-prefilteredDF[,c(1:requiredCols)]
      
      prefilteredType<-paste("prefiltered_minPts_",minPoints,"_distance_",dRadius,sep="")
      
      filePath<-file.path(outputFileDir,"dataDetailed",mutationType,prefilteredType)
      ...
      fileName<-paste(tumorType,"_",mutationType,"_",prefilteredType,"_merge_variant_details.txt",sep="")
      fileName<-file.path(filePath,fileName)
      write.table(prefilteredDF,fileName,sep="\t",quote=FALSE,row.names = FALSE,col.names = TRUE)
